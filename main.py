    ...
    for e in entities:
        ...
        try:
            if e.dxftype() == "LINE":
                ...
                ax.plot(
                    [start.x, end.x], [start.y, end.y], color="black", linewidth=0.5
                )
            ...
            if e.dxftype() == "CIRCLE":
                ...
                ax.add_patch(circle)
                ax.plot(center.x, center.y, "o", color="black", markersize=2)
...
def add_crop_marks(
    ax: Axes, xlim: tuple[float, float], ylim: tuple[float, float], mark_len: float = 5
) -> None:
    ...
    for entity in msp:
        try:
            if entity.dxftype() == "LINE":
                ...
                def angle_in_arc(
                    angle: float, start_deg: float, end_deg: float
                ) -> bool:
    ...
    with PdfPages(pdf_path) as pdf:
        for row in range(rows):
            for col in range(cols):
                ...
                fig = plt.figure(
                    figsize=(paper_size_mm[0] / 25.4, paper_size_mm[1] / 25.4)
                )
                ax = fig.add_axes(
                    [
                        margin_mm / paper_size_mm[0],
                ...
                draw_entities(entities=msp, ax=ax, entities_to_draw=entities_to_draw)
                if add_marks:
...
def parse_paper_size(size_str: str) -> tuple[float, float]:
    ...
    raise ValueError(
        f"Invalid paper size: {size_str}. Use format 'width,height' or standard size like A4, A3, etc."
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-crop-marks", action="store_true", help="Disable crop marks on the output"
    )
    ...
    parser.add_argument(
        ...
        help="Provide a list of strings (default: apple banana cherry)",
    )
    ...
    try:
        ...
        dxf_to_pdf_tiled(
            ...
            entities_to_draw=args.entities_to_draw,
